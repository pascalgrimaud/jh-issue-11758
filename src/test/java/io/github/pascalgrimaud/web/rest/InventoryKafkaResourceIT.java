...
import io.github.pascalgrimaud.config.KafkaProperties;
...
class InventoryKafkaResourceIT {
    ...
    void setup() {
        ...
        InventoryKafkaResource kafkaResource = new InventoryKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/inventory-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/inventory-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
